...
path_transfer <- "T:/HolleRoennfeldt/"
...
trait_eco_df$overlap <- logit(trait_eco_df$overlap)
trait_eco_df$rel_abandonment <- logit(trait_eco_df$rel_abandonment)
trait_eco_df$rel_unfilling <- logit(trait_eco_df$rel_unfilling)
trait_eco_df$rel_stability <- logit(trait_eco_df$rel_stability)
trait_eco_df$rel_expansion <- logit(trait_eco_df$rel_expansion)
trait_eco_df$rel_pioneering <- logit(trait_eco_df$rel_pioneering)
...
# overlap
null_overlap <- phylolm(overlap ~ 1, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)

# full models
lm_overlap <- phylolm(overlap ~ mean_height + mean_seedmass + growth_form + lifecycle + max_elev_range + lat_centroid + lon_centroid + range_size,
                      data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)

# step model
step_lm_overlap <- phylostep(lm_overlap$formula, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda")
...
# trait_eco_df$species <- sub(' ','_',trait_eco_df$species)

# stability
null_stability <- phylolm(rel_stability ~ 1, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)

# full models
lm_stability <- phylolm(rel_stability ~ mean_height + mean_seedmass + growth_form + lifecycle + max_elev_range + lat_centroid + lon_centroid + range_size, 
                        data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)
...
# unfilling
null_unfilling <- phylolm(rel_unfilling ~ 1, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)

# full models
lm_unfilling <- phylolm(rel_unfilling ~ mean_height + mean_seedmass + growth_form + lifecycle + max_elev_range + lat_centroid + lon_centroid + range_size, 
                        data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)
...
# expansion
null_expansion <- phylolm(rel_expansion ~ 1, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)

# full models
lm_expansion <- phylolm(rel_expansion ~ mean_height + mean_seedmass + growth_form + lifecycle + max_elev_range + lat_centroid + lon_centroid + range_size, 
                        data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)
...
# abandonment
null_abandonment <- phylolm(rel_abandonment ~ 1, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)

# full models
lm_abandonment <- phylolm(rel_abandonment ~ mean_height + mean_seedmass + growth_form + lifecycle + max_elev_range + lat_centroid + lon_centroid + range_size, 
                          data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-9)

# step model
step_lm_abandonment <- phylostep(lm_abandonment$formula, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda")
...
# pioneering
null_pioneering <- phylolm(rel_pioneering ~ 1, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-8)

# full models
lm_pioneering <- phylolm(rel_pioneering ~ mean_height + mean_seedmass + growth_form + lifecycle + max_elev_range + lat_centroid + lon_centroid + range_size, 
                         data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda", lower.bound = 10^-8)


# step model
step_lm_pioneering <- phylostep(lm_pioneering$formula, data = trait_eco_df, phy = phylo_subset[[1]], model = "lambda")
...
write.csv(results_TraitAnal_df, file = file.path("results/trait_analysis/results_TraitAnal_df.csv"), row.names=F)
...
traits_res <- read.csv("results/trait_analysis/results_TraitAnal_df.csv")
...
traits_res_tb_a <- traits_res %>%
  as_tibble() %>% 
  mutate(model = "abandonment", term = Trait, estimate = abandonment_coef, std.error = abandonment_stderr, statistic = abandonment_coef/abandonment_stderr, p.value = abandonment_p, varimp = abandonment_varimp) %>%
  select(model, term, estimate, std.error, statistic, p.value, varimp) %>% 
  filter(term %in% covariates)

traits_res_tb_p <- traits_res %>%
  as_tibble() %>% 
  mutate(model = "pioneering", term = Trait, estimate = pioneering_coef, std.error = pioneering_stderr, statistic = pioneering_coef/pioneering_stderr, p.value = pioneering_p, varimp = pioneering_varimp) %>%
  select(model, term, estimate, std.error, statistic, p.value, varimp) %>% 
  filter(term %in% covariates)
...
p <- dwplot(traits_res_tb_all, 
  ...
  ylab("") + 
  scale_color_manual(values = c("abandonment" = "#C76967", "unfilling" = "#FFE875", "stability" = "#A3DDEF","expansion" = "#87CF87", "pioneering" = "#927290"), name = "model")


ggsave("plots/results/Coefficient estimate.pdf", p, 
       bg = "transparent",
       width = 6,
       height = 5,
       units = "cm")
