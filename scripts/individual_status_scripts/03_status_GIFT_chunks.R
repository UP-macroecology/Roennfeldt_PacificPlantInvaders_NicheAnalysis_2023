...
GIFT_polygons <- GIFT_shapes(unique(GIFT_status$entity_ID), GIFT_version = "beta")
...
load("data/status_assignment/Gift_status.RData")
tdwg <- st_read("data/tdwg/geojson/level3.geojson")

# get number of species for which GIFT status was available
specs_initial <- unique(GIFT_status$species) # 3700 species in total  

# when doing the first status assignment 
specs_gift <- specs_initial[1:50]
...
specs_done <- unique(occ_GIFT_status$species)

specs_left <- setdiff(specs_initial, specs_done)

(specs_gift <- specs_left[1:100])


# alternative: occ_GIFT_status_1
occ_GIFT_status_1 <- foreach(s = 1:length(specs_gift), .packages = c("dplyr", "sf"),
...
# load("data/status_assignment/occ_gift_status_intermediate.RData")
# load("data/status_assignment/occ_gift_status_intermediate_1.RData")

# split into smaller chunks as this demands a lot of memory and would be too much to do at once on the HPC
specs_exist <- unique(occ_GIFT_status$species)
rm(occ_GIFT_status)

specs_done <- c(unique(occ_GIFT_status_1$species), specs_exist)

specs_left <- setdiff(specs_initial, specs_done)

(specs_gift <- specs_left)

occ_GIFT_status_new <- foreach(s = 1:length(specs_gift), .packages = c("dplyr", "sf"),
                                   .combine = "rbind", .verbose = TRUE) %dopar% {
...
occ_GIFT_status_1 <- rbind(occ_GIFT_status_1, occ_GIFT_status_new)
rm(occ_GIFT_status_new)

save(occ_GIFT_status_1, file = "data/status_assignment/occ_gift_status_intermediate_1.RData")

## and repeat



# save(occ_GIFT_status_1, file = paste0(path_imp, "/occ_GIFT_status/occ_GIFT_status_1.RData"))
...
occ_GIFT_status <- rbind(occ_GIFT_status, occ_GIFT_status_1)
