#' ---------------------------
#
# Purpose of script: thin presences points in the native range, sample pseudo-absences (within 50 km buffer) and thin them as well
...
load(paste0(path_data, "/regional_occs/criterion_1/occ_count_crit_1.RData"))
...
foreach(spp_index = 1:length(spp), .packages = c("terra", "tidyverse", "sf", "purrr", "furrr", "sfheaders")) %dopar% {
  try({
    ...
    t <- loadRData(paste0(path_data, "/regional_occs/criterion_1/native/nat_occs",spp[spp_index],".RData")) # the objects all have different names depending on the target region
    ...
    load(paste0(path_data, "/output/coords_final_nat_rev/coords_final_nat_200_",spp[spp_index],".RData")) 
    ...
    abs_thinned_50 <- thin(abs_coords_50_sf, thin_dist = 3000, runs = 1, ncores = 1)
    ...
    save(coords_final_nat_50, file = paste0(path_data, "/output/buffer_sensitivity/coords_final_nat/coords_final_nat_50_", spp[spp_index],".RData"))
