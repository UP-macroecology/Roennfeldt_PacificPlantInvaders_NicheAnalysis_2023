...
occ_count_2 <- data.frame(matrix(ncol = 10, nrow = 0))
colnames(occ_count_2) <- c("species", "native_occs", "pac_occs", "eur_occs", "afr_occs", "ate_occs", "atr_occs", "aus_occs", "nam_occs", "sam_occs")
...
occ_count_2 <- foreach(spp_index = 1:length(spp), .combine = "rbind") %dopar%
  try({
    ...
    if(nr_nat >= 20){
      save(nat, file = paste0(path_imp,"/output/regional_occs_2/nat1_",spp[spp_index],".RData"))
      ...
      if(nr_pac >= 20){# get coords and use them to select occs from the intr subset
        ...
        save(pac_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_pac_",spp[spp_index],".RData"))
        ...
        for(reg in regs){
          ...
          if(region_name == "EUROPE"){
            ...
            if(nr_eur >= 20){save(eur_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_eur_",spp[spp_index],".RData"))}
          ...
          if(region_name == "AFRICA"){
            ...
            if(nr_afr >= 20){save(afr_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_afr_",spp[spp_index],".RData"))}
          } # end outer if Africa
          ...
          if(region_name == "ASIA-TEMPERATE"){
            ...
            if(nr_ate >= 20){save(ate_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_ate_",spp[spp_index],".RData"))}
          ...
          if(region_name == "ASIA-TROPICAL"){
            ...
            if(nr_atr >= 20){save(atr_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_atr_",spp[spp_index],".RData"))}
          } # end outer if Asia (temperate)
          
          if(region_name == "AUSTRALASIA"){
            aus_df <- subset(intr, tdwg_l1_code == reg)
            nr_aus <- nrow(aus_df)
            if(nr_aus >= 20){save(aus_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_aus_",spp[spp_index],".RData"))}
          } # end outer if Australasia
          
          if(region_name == "NORTHERN AMERICA"){
            nam_df <- subset(intr, tdwg_l1_code == reg)
            nr_nam <- nrow(nam_df)
            if(nr_nam >= 20){save(nam_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_nam_",spp[spp_index],".RData"))}
          } # end outer if Northern America
          
          if(region_name == "SOUTHERN AMERICA"){
            sam_df <- subset(intr, tdwg_l1_code == reg)
            nr_sam <- nrow(sam_df)
            if(nr_sam){save(sam_df, file = paste0(path_imp,"/output/regional_occs_2/intr2_sam_",spp[spp_index],".RData"))}
...
save(occ_count_2, file = file.path(path_imp, "output/occ_count_criterion2.RData"))