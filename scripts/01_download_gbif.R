...
usda_weeds = read_csv(file.path(path_mnt, "USDA_Hawaii_noxious_weeds.csv")) %>% # Where are those species in Michael's list??
...
inv_specs = read.csv(file.path(path_mnt, "Pacific_Invaders_GIFT_22_01.csv"), sep = ";", dec = ".") %>% 
  dplyr::select(Species) %>% 
  distinct() %>% 
  drop_na() %>% 
  pull(Species)
